...
addRequestAdmin = (form) => {
  ...
  if (formData.get("customer_id") !== "") {
      if (formData.get("sender_name").trim() !== "") {
          if (formData.get("sender_phone").trim() !== "") {
              if (formData.get("sender_address").trim() !== "") {
                  if (formData.get("weight").trim() !== "") {
                      if (formData.get("total_fee").trim() !== "") {
                          if (formData.get("rec_name").trim() !== "") {
                              if (formData.get("rec_phone").trim() !== "") {
                                  if (formData.get("rec_address").trim() !== "") {
                                      $.ajax({
                                          method: "POST",
                                          url: "../server/api.php?function_code=addRequestAdmin",
                                          data: formData,
                                          success: function(data) {
                                              console.log(data); // Check response in console
                                              successToastRedirect("courier.php"); // Redirect after success
                                          },
                                          error: function(error) {
                              ...
                              errorMessage("Please Enter Receiver Name");
                          }
                      } else {
                  ...
                  errorMessage("Please Enter Sender Address");
              }
          } else {
    ...
    if (formData.get("new_password").trim() != "") {
      if (formData.get("confirm_new_password").trim() != "") {
        if (
          ...
          if (
            checkPasswordAdmin(formData.get("current_password"),formData.get("email")) > 0
          ) {
            var data = {
