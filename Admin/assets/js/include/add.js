...
addBranch = (form) => {
  ...
  if (fd.get("branch_name").trim() != "") {
    $.ajax({
      ...
      success: function ($data) {
        if ($data > 0) {
          errorMessage("This Branch Already Exist..");
        } else {
          successToast();
        }
      },
...
addArea = (form) => {
  ...
  if (fd.get("area_name").trim() != "") {
    $.ajax({
      ...
      success: function ($data) {
        if ($data > 0) {
          errorMessage("This Area Already Exist..");
        } else {
          successToast();
        }
      },
      ...
      if (fd.get("price").trim() != "") {
        $.ajax({
          ...
          success: function ($data) {
            console.log($data);
            if ($data > 0) {
              normalAlertNoReload("error", "The Location Price is Already Exist..");
            } else {
              successToast();
            }
          },
                      ...
                      $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (response.includes("Email already exists")) {
                                ...
                                successToastRedirect("customer.php");
                            }
                        },
                        cache: false,
                        contentType: false,
