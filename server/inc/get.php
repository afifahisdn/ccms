...
function getBillByTracking($tracking_id)
{
    include "connection.php";
...
function getAllTrackingByID($tracking_id)
{
    include "connection.php";

    $viewcat = "SELECT * FROM request WHERE is_deleted = 0 AND tracking_id = '$tracking_id' ORDER BY date_updated DESC";
    return mysqli_query($con, $viewcat);
}
function getAllTracking()
{
    ...
    $viewcat =
        "SELECT * FROM request join customer on customer.customer_id = request.customer_id WHERE request.is_deleted = 0 ORDER BY date_updated DESC";
    return mysqli_query($con, $viewcat);
}
...
function checkemployeetByEmail($email)
{
    ...
    } elseif (mysqli_num_rows($cus_res) > 0) {
...
function checkBranchByName($branch_name)
{
    include "connection.php";

    $q1 = "SELECT * FROM branch WHERE branch_name = '$branch_name' AND is_deleted = 0";
    $res = mysqli_query($con, $q1);
    return mysqli_num_rows($res);
}
